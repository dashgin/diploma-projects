    ...
    def access_token(self) -> str:
        if not self._access_token:
            ...
            response = requests.post(
                f"{self.backend_url}/api/v1/login/access-token",
...
class AIFeedbackTester:
    ...
    def test_health_check(self) -> bool:
        ...
        try:
            response = requests.get(
                f"{self.config.backend_url}/api/v1/utils/health-check"
            )
    ...
    def create_test_attempt(self, quiz_id: int = 2) -> Optional[int]:
        ...
        attempt_data = {"quiz_id": quiz_id, "is_completed": False}
    ...
    def create_test_response(
        self, attempt_id: int, question_id: int, answer_type: str = "incorrect"
    ) -> Optional[int]:
        """Create a test response"""
        print(
            f"✍️ Creating test response for attempt {attempt_id}, question {question_id}..."
        )
    ...
    def check_feedback_status(
        self, response_id: int, max_retries: int = 10
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    if feedback_data:  # Not null
                        ...
                        print(
                            f"  📊 Confidence: {feedback_data.get('confidence_score', 0):.2%}"
                        )
                        print(f"  🏷️ Error types: {feedback_data.get('error_type', [])}")
                        return feedback_data

                print(
                    f"  ⏳ Attempt {attempt + 1}/{max_retries} - No feedback yet, waiting..."
                )
                time.sleep(2)
    ...
    def test_ai_service_direct(self, response_id: int = 28) -> bool:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                print(
                    f"  📝 Feedback: {feedback['feedback']['feedback_text'][:100]}..."
                )
                print(
                    f"  🎯 Confidence: {feedback['feedback']['confidence_score']:.2%}"
                )
                return True
            else:
...
def main():
    ...
    parser.add_argument(
        "--create-test-data", action="store_true", help="Create test data only"
    )
    parser.add_argument(
        "--test-feedback", action="store_true", help="Test feedback generation"
    )
    parser.add_argument(
        "--response-id", type=int, help="Response ID for feedback testing"
    )
    parser.add_argument(
        "--cleanup", action="store_true", help="Show cleanup information"
    )
    parser.add_argument(
        "--health-check", action="store_true", help="Check service health"
    )
    parser.add_argument(
        "--ai-direct", action="store_true", help="Test AI service directly"
    )
    ...
    if args.health_check:
        ...
        if question_id:
            ...
            if attempt_id:
                response_id = tester.create_test_response(
                    attempt_id, question_id, "incorrect"
                )
                print(
