...
const EditQuestion = ({ question }: EditQuestionProps) => {
  ...
  } = useForm<QuestionUpdateForm>({
    mode: "onBlur",
    criteriaMode: "all",
    defaultValues: {
      text: question.text,
      question_type: question.question_type,
      order_position: question.order_position ?? undefined,
  ...
  const mutation = useMutation({
    mutationFn: (data: QuestionUpdate) =>
      QuestionsService.partiallyUpdateQuestion({
  ...
  const onSubmit: SubmitHandler<QuestionUpdateForm> = async (data) => {
    mutation.mutate(data as QuestionUpdate)
  }
    ...
    >
      ...
      <DialogContent>
        <form onSubmit={handleSubmit(onSubmit)}>
          ...
          <DialogBody>
            <Text mb={4}>Update the question details.</Text>
            <VStack gap={4}>
              <Field
                required
                invalid={!!errors.text}
