...
def create_knowledge_areas(session: Session) -> list[KnowledgeArea]:
    ...
    areas = [
        KnowledgeArea(name="Mathematics", description="Mathematical concepts and problem solving"),
        KnowledgeArea(name="Science", description="Scientific principles and theories"),
        KnowledgeArea(name="Programming", description="Computer programming and software development"),
        KnowledgeArea(name="History", description="Historical events and periods"),
        KnowledgeArea(name="Languages", description="Language learning and linguistics"),
    ]
...
def create_quiz(
    session: Session, creator_id: int, title: str, area_id: int, questions_data: list[dict]
) -> Quiz:
...
def enroll_students(session: Session, class_id: int, student_ids: list[int]) -> list[StudyClassEnrollment]:
...
def assign_quiz(session: Session, quiz_id: int, student_id: int, class_id: int = None) -> QuizAssignment:
...
def create_attempt(session: Session, quiz_id: int, student_id: int, assignment_id: int = None) -> StudentAttempt:
...
def create_responses(session: Session, attempt_id: int, questions: list[QuizQuestion]) -> list[StudentResponse]:
...
def main() -> None:
    ...
    with Session(engine) as session:
        ...
        programming_class = create_study_class(session, educator.id, "Programming Fundamentals")
        
        # Enroll students in classes
        print("Enrolling students...")
        enroll_students(session, math_class.id, student_ids[:3])  # First 3 students in math
        enroll_students(session, programming_class.id, student_ids[2:])  # Last 3 students in programming
