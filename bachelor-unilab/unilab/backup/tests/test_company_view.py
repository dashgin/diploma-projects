...
def test_permissions(
    client, users, status_codes, request, create_companies, django_user_model
):
    ...
    for user in users:
        ...
        response = client.get(
            "/api/companies/", HTTP_AUTHORIZATION=f"Bearer {wrong_token}"
        )
        ...
        response = client.get(
            "/api/companies/1/", HTTP_AUTHORIZATION=f"Bearer {access_token}"
        )
        assert response.status_code == status_codes[4]

        # Check with incorrect credentials
        response = client.get(
            "/api/companies/1/", HTTP_AUTHORIZATION=f"Bearer {wrong_token}"
        )
