...
def test_permissions(
    client, users, status_codes, request, create_posts, django_user_model
):
    ...
    for user in users:
        ...
        response = client.get(
            "/api/posts/", HTTP_AUTHORIZATION=f"Bearer {access_token}"
        )
        ...
        post_url = (
            response.json()["url"]
            if user.user_type is not None
            else f"/api/posts/{posts[3].id}/"
        )

        # Check the GET on post-detail
        response = client.get(
            f"/api/posts/{posts[0].id}/", HTTP_AUTHORIZATION=f"Bearer {access_token}"
        )
...
def test_fields(client, request, field, value, users, create_posts):
    ...
    for user in users:
        ...
        for post in posts:
            response = client.get(
                f"/api/posts/{post.id}/", HTTP_AUTHORIZATION=f"Bearer {access_token}"
            )
            assert response.status_code == 200
            ...
            if field == "url":
                ...
                assert (
                    response.json()["image"]
                    == f"http://testserver{getattr(post, 'image').url}"
                )
            else:
