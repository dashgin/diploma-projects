...
def test_permissions(client, users, status_codes, request):
    ...
    for user in users:
        ...
        response = client.post(
            "/api/token/",
            {"email": user.email, "password": "4321"},
        ...
        response = client.get(
            "/api/users/", HTTP_AUTHORIZATION=f"Bearer {access_token}"
        )
        ...
        response = client.get(
            "/api/users/1/", HTTP_AUTHORIZATION=f"Bearer {access_token}"
        )
...
def test_fields(client, field, value, create_users_admins):
    ...
    for admin in create_users_admins:
        ...
        for i in range(admin_count):
            response = client.get(
                f"/api/users/{i + 1}/", HTTP_AUTHORIZATION=f"Bearer {access_token}"
            )
            assert response.status_code == 200
            ...
            if field == "url":
                ...
                assert response.json()[field] == list(
                    getattr(create_users_admins[i], field).all()
                )
            elif field == "image":
                assert (
                    response.json()["image"]
                    == f"http://testserver{getattr(create_users_admins[i], 'image').url}"
                )
            else:
