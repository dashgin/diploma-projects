...
def test_permissions(
    client, users, status_codes, request, create_comments, django_user_model
):
    ...
    for user in users:
        ...
        response = client.get(
            "/api/comments/", HTTP_AUTHORIZATION=f"Bearer {access_token}"
        )
        ...
        comment_url = (
            response.json()["url"]
            if user.user_type is not None
            else f"/api/comments/{random.choice(comments).id}/"
        )

        # Check the GET on comment-detail
        response = client.get(
            f"/api/comments/{random.choice(comments).id}/",
            HTTP_AUTHORIZATION=f"Bearer {access_token}",
        )
        ...
        response = client.patch(
            f"http://testserver/api/comments/{comments[0].id}/",
            {"content": "a changed content"},
            HTTP_AUTHORIZATION=f"Bearer {access_token}",
            content_type="application/json",
        )
...
@pytest.mark.parametrize(
    "users", ["create_users_admins", "create_users_companies", "create_users_students"]
)
        ...
        for comment in comments:
            ...
            for field in fields:
                if field == "owner":
                    assert (
                        response.json()["owner"]
                        == f"http://testserver/api/users/{comment.owner.id}/"
                    )
                elif field == "post":
                    assert (
                        response.json()["post"]
                        == f"http://testserver/api/posts/{comment.post.id}/"
                    )
                elif field == "publish_date":
                    assert response.json()[
                        "publish_date"
                    ] == comment.publish_date.strftime("%Y-%m-%d")
                elif field == "url":
                    assert (
                        response.json()["url"]
                        == f"http://testserver/api/comments/{comment.id}/"
                    )
                else:
