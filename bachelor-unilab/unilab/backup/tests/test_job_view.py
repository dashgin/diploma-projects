...
def test_permissions(
    client, users, status_codes, request, django_user_model, create_jobs
):
    users = request.getfixturevalue(users)
    companies = (
        create_jobs["companies"]["admins"] + create_jobs["companies"]["companies"]
    )
    ...
    for user in users:
        ...
        response = client.get(
            "/api/companies/", HTTP_AUTHORIZATION=f"Bearer {access_token}"
        )
        response = client.post(
            "/api/jobs/",
            {
        ...
        response = client.get(
            f"/api/jobs/{jobs[0].id}/", HTTP_AUTHORIZATION=f"Bearer {access_token}"
        )
...
def test_fields(
    client, field, value, create_users_admins, create_users_companies, create_jobs
):
    ...
    for user in users:
        ...
        for job in jobs:
            response = client.get(
                f"/api/jobs/{job.id}/", HTTP_AUTHORIZATION=f"Bearer {access_token}"
            )
            assert response.status_code == 200
            ...
            if field == "url":
                ...
                assert response.json()["publish_date"] == getattr(
                    job, "publish_date"
                ).strftime("%Y-%m-%d")
            elif field == "hours_per_week":
                assert float(response.json()["hours_per_week"]) == float(
                    getattr(job, field)
                )
            else:
