...
class Post(models.Model):
    ...
    company = models.ForeignKey(
        "Company", blank=True, on_delete=models.CASCADE, null=True
    )
...
class User(AbstractBaseUser, PermissionsMixin):
    ...
    image = models.ImageField(
        upload_to="user_image/%Y/%m/%D/", default="defaults/profile.png"
    )
    ...
    voted_posts = models.ManyToManyField(
        Post, blank=True, through="Vote", related_name="votes"
    )
...
class ExternalProfile(models.Model):
    ...
    owner = models.ForeignKey(
        "UserData", related_name="external_profiles", on_delete=models.CASCADE
    )
...
class UniversityCourse(models.Model):
    ...
    owner = models.ForeignKey(
        "UserData", related_name="university_courses", on_delete=models.CASCADE
    )
...
class Certification(models.Model):
    ...
    owner = models.ForeignKey(
        "UserData", related_name="certifications", on_delete=models.CASCADE
    )
    title = models.CharField(max_length=200)
    description = models.CharField(max_length=200, null=True, blank=True)
    proof = models.ImageField(
        upload_to="certification_proof/%Y/%m/%D/", null=True, blank=True
    )
...
class EducationData(models.Model):
    ...
    user_data = models.ForeignKey(
        "UserData", related_name="education_data", on_delete=models.CASCADE
    )
    ...
    image = models.ImageField(
        upload_to="education_image/%Y/%m/%D/", default="defaults/university.jpg"
    )
...
class SkillData(models.Model):
    ...
    user_data = models.ForeignKey(
        "UserData", related_name="skill_data", on_delete=models.CASCADE
    )
...
class Company(models.Model):
    ...
    publish_date = models.DateField(default=timezone.now)
    owner = models.ForeignKey(
        settings.AUTH_USER_MODEL, related_name="companies", on_delete=models.CASCADE
    )
    admins = models.ManyToManyField(User, blank=True, through="CompanyAdmin")
    image = models.ImageField(
        upload_to="company_image/%Y/%m/%D/", default="defaults/company.jpg"
    )
    ...
    class Industries(models.IntegerChoices):
        ENERGY = (
            1,
            _("Energy, Utilities and Resources"),
        )
        GOVERNMENT = (
            2,
            _("Government and Public Sector"),
        )
        PHARMA = (
            3,
            _("Pharmaceuticals and Life Sciences"),
        )
        ESTATE = (
            4,
            _("Real Estate"),
        ...
        MANUFACTURE = (
            8,
            _("Industrial Manufacturing"),
        )
        RETAIL = (
            9,
            _("Retail and Consumer Goods"),
        )
...
class CompanyAdmin(models.Model):
    ...
    user = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name="company_admins"
    )
...
class University(models.Model):
    ...
    admins = models.ManyToManyField(
        User, blank=True, through="UniversityAdmin", related_name="university_admin_set"
    )
    image = models.ImageField(
        upload_to="company_image/%Y/%m/%D/", default="defaults/university.jpg"
    )
...
class UniversityAdmin(models.Model):
    ...
    user = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name="university_admins"
    )
...
class Vote(models.Model):
    ...
    UPVOTE = 1
    DOWNVOTE = 2
    TYPE_CHOICES = [(UPVOTE, _("Upvote")), (DOWNVOTE, _("Downvote"))]
...
class Job(models.Model):
    ...
    applicants = models.ManyToManyField(
        User, blank=True, through="Application", related_name="job"
    )
